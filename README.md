...
4. **POST** `/crm/addContact`
   ...
   * **Responses**:
     ...
     * `500 Internal Server Error` – Save or sync failure

### Frontend

* **auth.html**: Registration and login forms
* **profile.html**: Displays user profile and provides interface to fetch CRM contacts and add new contacts
* **contacts\_dashboard.html**: Contacts dashboard to add a new contact or fetch all contacts from Bitrix24 and MySQL
...
1. **Clone the Repository**:
   ...
   ```bash
   git clone https://github.com/migz19/reyada.git
   cd reyada
   ```

2. **Database**: Create a MySQL database (e.g., `myapp`).

3. **Configuration**: Update `application.properties` with your MySQL and Bitrix24 API credentials.
